...
class EmbedMetaAttenSeq(nn.Module):
    ...
    def forward(self, seqs, metadata):
        ...
        # latent_seqs = self.rnn(seqs)[0]
        # print("latent_seqs after GRU:", latent_seqs)  # 打印 GRU 输出
        #
        # latent_seqs = self.attn_layer(torch.transpose(latent_seqs, 0, 1)).sum(0)
        # print("latent_seqs after attention:", latent_seqs)  # 打印注意力层后的输出
        #
        # meta_emb = self.emb_layer(metadata)
        # print("meta_emb:", meta_emb)  # 打印元数据嵌入
        #
        # out = self.out_layer(torch.cat([latent_seqs, meta_emb], dim=1))
        # print("final output:", out)  # 打印最终输出
...
class TwoSepFNP(nn.Module):
    """
    Modified Functional Neural Process for regression with two encoders
    """
    ...
    def __init__(
        ...
        # Pairwise function to determine edge probabilities
        self.pairwise_g_logscale = nn.Parameter(torch.tensor(math.log(math.sqrt(self.dim_u))))
        self.pairwise_g = lambda x: -0.5 * torch.sum(torch.pow(x[:, self.dim_u:] - x[:, :self.dim_u], 2), 1, keepdim=True) / self.pairwise_g_logscale.exp()

        # Input transformations
        self.cond_trans1 = self._build_encoder(dim_x1, dim_h, n_layers)  # First encoder
        self.cond_trans2 = self._build_encoder(dim_x2, dim_h, n_layers)  # Second encoder

        # p(u|x)
        self.p_u = nn.Linear(dim_h, 2 * dim_u)
        # q(z|x)
        self.q_z = nn.Linear(dim_h, 2 * dim_z)

        if use_ref_labels:
            self.trans_cond_y = nn.Linear(dim_y, 2 * dim_z)

        self.output_params = [
            nn.Parameter(torch.randn(self.nodes, self.dim_z + self.dim_x1 + self.dim_x2, self.dim_h) / math.sqrt(self.dim_h)),
            nn.Parameter(torch.randn(self.nodes, self.dim_h, self.dim_h) / math.sqrt(self.dim_h)),
            nn.Parameter(torch.randn(self.nodes, self.dim_h, 2 * dim_y) / math.sqrt(self.dim_h))
        ]

        if self.add_atten:
            self.atten_layer = LatentAtten(self.dim_h)

    def _build_encoder(self, dim_x, dim_h, n_layers):
        init = [nn.Linear(dim_x, dim_h), nn.ReLU()]
        for _ in range(n_layers - 1):
            init += [nn.Linear(dim_h, dim_h), nn.ReLU()]
        return nn.Sequential(*init)

    def forward(self, XR1, XR2, XM1, XM2, yM, kl_anneal=1.0):
        # Process the two parts of input separately
        H1 = self.cond_trans1(XR1)
        H2 = self.cond_trans2(XR2)
        H_all = torch.cat([H1, H2], dim=-1)

        # p(u|x) for both parts
        pu_mean_all, pu_logscale_all = torch.split(self.p_u(H_all), self.dim_u, dim=1)
        pu = Normal(pu_mean_all, pu_logscale_all)
        u = pu.rsample()
        ...
        # Sample G and A based on the hierarchical structure
        G = sample_Clique(u[:XR1.size(0)], self.pairwise_g, training=self.training)
        A = sample_bipartite(u[XR1.size(0):], u[:XR1.size(0)], self.pairwise_g, training=self.training)

        if self.add_atten:
            atten = self.atten_layer(H2, H1)
            A = A * atten

        pz_mean_all = torch.mm(self.norm_graph(torch.cat([G, A], dim=0)), qz_mean_all[:XR1.size(0)])
        pz_logscale_all = torch.mm(self.norm_graph(torch.cat([G, A], dim=0)), qz_logscale_all[:XR1.size(0)])
        pz = Normal(pz_mean_all, pz_logscale_all)

        # Latent variable optimization
        pqz_all = pz.log_prob(z) - qz.log_prob(z)
        log_pqz_M = torch.sum(pqz_all[XR1.size(0):])

        final_rep = z if not self.use_plus else torch.cat([z, u], dim=1)

        # Merge with shared information (sR)
        sR = XR1.mean(dim=0)  # Shared representation (mean over XR1)
        sR = sR.repeat(final_rep.shape[0], 1)
        final_rep = torch.cat([sR, final_rep], dim=-1)
    ...
    def predict(self, XR1, XR2, x_new1, x_new2, sample=True):
        H1 = self.cond_trans1(torch.cat([XR1, x_new1], dim=0))
        H2 = self.cond_trans2(torch.cat([XR2, x_new2], dim=0))
        H_all = torch.cat([H1, H2], dim=-1)
        ...
        # Sample G and A based on the hierarchical structure
        A = sample_bipartite(u[XR1.size(0):], u[:XR1.size(0)], self.pairwise_g, training=False)

        pz_mean_all = torch.mm(self.norm_graph(A), qz_mean_all)
        pz_logscale_all = torch.mm(self.norm_graph(A), qz_logscale_all)
        pz = Normal(pz_mean_all, pz_logscale_all)

        final_rep = z if not self.use_plus else torch.cat([z, u[XR1.size(0):]], dim=1)
        sR = XR1.mean(dim=0).repeat(final_rep.shape[0], 1)
        final_rep = torch.cat([sR, final_rep], dim=-1)
